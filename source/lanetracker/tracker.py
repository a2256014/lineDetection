...
class LaneTracker(object):
    ...
    def process(self, frame, draw_lane=True, draw_statistics=True):
        ...
        if draw_statistics:
            ...
            text_x = 20 + 20 + w + w + 20
            self.draw_text(frame, 'radius curve:  {} m'.format(self.radius_of_curvature()), text_x, 80)
            self.draw_text(frame, 'Distance from center (left):       {:.1f} m'.format(self.left.camera_distance()), text_x, 140)
            self.draw_text(frame, 'Distance from center (right):      {:.1f} m'.format(self.right.camera_distance()), text_x, 200)
